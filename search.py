...
def boost(boost_array):
    ...
    term10 = "sinhala_metaphor^{}".format(boost_array[9])
    term11 = "sinhala_source^{}".format(boost_array[10])
    term12 = "sinhala_target^{}".format(boost_array[11])
    term13 = "sinhala_interpretation^{}".format(boost_array[12])
...
def search(phrase,search_type):
    ...
    if (search_type == "anywhere"):
        ...
        if (isEnglish(phrase)==False):
            query_body = queries.multi_match(phrase, ["sinhala_metaphor","sinhala_source", "sinhala_target"])
        else:
            return("Cannot search metaphors in english")
    
    elif (search_type == "lyricist_only"):
        if (isEnglish(phrase)==True):
            query_body = queries.single_field_full_match(phrase,"english_lyricist")
        else:
            query_body = queries.single_field_full_match(phrase,"sinhala_lyricist")
